  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-white">Tools & Integrations</h1>
          ...
          <CardBody className="divide-y divide-surface-border p-0">
            {tools.length === 0 ? (
              <div className="flex flex-col items-center gap-3 px-5 py-12 text-center">
                <p className="text-sm text-white/50">No tools configured yet.</p>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add a tool" subtitle="Give an agent a new capability, gated by an execution tier.">
        <form onSubmit={createTool} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-white/60">Execution tier</label>
              <select
                value={executionTier}
          ...
          <p className="text-[11px] leading-relaxed text-white/35">{TIERS.find((t) => t.value === executionTier)?.label}</p>
          <div>
            <label className="mb-1 block text-xs font-medium text-white/60">Credential / API key (optional)</label>
            <input
              type="password"
