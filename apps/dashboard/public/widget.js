"use strict";(()=>{(function(){let b=document.currentScript,p=b?.dataset.agentId;if(!p){console.error("[chat-widget] missing data-agent-id on the embed <script> tag.");return}let x=b?.dataset.apiBase??new URL(b?.src??"",location.href).origin,v=`chat-agent:${p}`,f=JSON.parse(localStorage.getItem(v)??"{}"),w=f.sessionCookie??crypto.randomUUID(),m=f.conversationId;function L(){localStorage.setItem(v,JSON.stringify({conversationId:m,sessionCookie:w}))}let u=document.createElement("div");u.id="chat-agent-widget-root",document.body.appendChild(u);let n=u.attachShadow({mode:"open"});n.innerHTML=`
    <style>${q()}</style>
