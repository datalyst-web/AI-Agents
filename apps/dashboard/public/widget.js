"use strict";(()=>{var P={DARK:{"--panel-bg":"#0c0d13","--text-primary":"#ffffff","--text-secondary":"rgba(255,255,255,0.45)","--text-muted":"rgba(255,255,255,0.28)","--header-border":"rgba(255,255,255,0.07)","--bubble-agent-bg":"rgba(255,255,255,0.06)","--bubble-agent-border":"rgba(255,255,255,0.06)","--bubble-agent-text":"#f2f3f7","--input-bg":"rgba(255,255,255,0.06)","--input-border":"rgba(255,255,255,0.1)","--input-focus-bg":"rgba(255,255,255,0.08)","--input-text":"#ffffff","--input-placeholder":"rgba(255,255,255,0.32)","--composer-border":"rgba(255,255,255,0.07)","--composer-bg":"rgba(255,255,255,0.02)","--close-icon":"rgba(255,255,255,0.4)","--close-hover-bg":"rgba(255,255,255,0.08)","--scrollbar-thumb":"rgba(255,255,255,0.12)","--panel-shadow":"0 24px 70px rgba(0,0,0,0.5), 0 4px 16px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.07) inset","--header-glow":"rgba(53,189,240,0.25)"},LIGHT:{"--panel-bg":"#ffffff","--text-primary":"#0f1330","--text-secondary":"rgba(15,19,48,0.55)","--text-muted":"rgba(15,19,48,0.4)","--header-border":"rgba(15,19,48,0.08)","--bubble-agent-bg":"rgba(15,19,48,0.045)","--bubble-agent-border":"rgba(15,19,48,0.07)","--bubble-agent-text":"#0f1330","--input-bg":"rgba(15,19,48,0.035)","--input-border":"rgba(15,19,48,0.12)","--input-focus-bg":"rgba(15,19,48,0.05)","--input-text":"#0f1330","--input-placeholder":"rgba(15,19,48,0.38)","--composer-border":"rgba(15,19,48,0.08)","--composer-bg":"rgba(15,19,48,0.015)","--close-icon":"rgba(15,19,48,0.45)","--close-hover-bg":"rgba(15,19,48,0.06)","--scrollbar-thumb":"rgba(15,19,48,0.14)","--panel-shadow":"0 24px 70px rgba(20,27,77,0.16), 0 4px 16px rgba(20,27,77,0.1), 0 0 0 1px rgba(15,19,48,0.06) inset","--header-glow":"rgba(53,189,240,0.14)"}};(function(){let f=document.currentScript,l=f?.dataset.agentId;if(!l){console.error("[chat-widget] missing data-agent-id on the embed <script> tag.");return}let m=f?.dataset.apiBase??new URL(f?.src??"",location.href).origin,N=f?.dataset.position==="left"?"left":"right",S=`chat-agent:${l}`,h=JSON.parse(localStorage.getItem(S)??"{}"),I=h.sessionCookie??crypto.randomUUID(),s=h.conversationId,E=h.csatSubmittedFor;function C(){localStorage.setItem(S,JSON.stringify({conversationId:s,sessionCookie:I,csatSubmittedFor:E}))}let d=document.createElement("div");d.id="chat-agent-widget-root",d.style.display="none",document.body.appendChild(d);let r=d.attachShadow({mode:"open"});r.innerHTML=`
  ...
  `;let p=r.querySelector(".launcher"),x=r.querySelector(".panel"),j=r.querySelector(".close"),b=r.querySelector(".messages"),g=r.querySelector(".confirmation"),v=r.querySelector(".csat"),y=r.querySelectorAll(".csat-star"),q=r.querySelector(".csat-skip"),Y=r.querySelector(".composer"),u=r.querySelector("input"),$=r.querySelector(".composer button"),U=r.querySelector(".name"),M=r.querySelector(".avatar"),O=r.querySelector(".avatar-fallback");u.addEventListener("input",()=>{$.disabled=u.value.trim().length===0});let i,w=!1;function R(e){let t=P[e??"LIGHT"];for(let[a,o]of Object.entries(t))d.style.setProperty(a,o)}async function D(){let e=await fetch(`${m}/v1/widget-config/${l}`);if(!e.ok)throw Object.assign(new Error(`widget-config fetch failed: ${e.status}`),{status:e.status});i=await e.json(),R(i.theme),U.textContent=i.name;let t=i.logoUrl||i.avatarUrl;t&&(M.src=t,M.hidden=!1,O.hidden=!0),h.conversationId||c("agent",i.greeting)}let z;function L(e){w=e,clearTimeout(z),p.classList.toggle("open",e),e&&p.classList.remove("invite"),e?(x.hidden=!1,requestAnimationFrame(()=>x.classList.add("open")),u.focus()):(x.classList.remove("open"),z=setTimeout(()=>{w||(x.hidden=!0)},200))}function F(){return!s||E===s||!v.hidden?!1:(v.hidden=!1,y.forEach(e=>e.classList.remove("filled")),!0)}async function H(e){if(v.hidden=!0,e!==null&&s&&i){E=s,C();try{await fetch(`${m}/v1/chat/${l}/conversations/${s}/csat`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${i.widgetToken}`},body:JSON.stringify({score:e})})}catch{}}L(!1)}y.forEach(e=>{e.addEventListener("mouseenter",()=>{let t=Number(e.dataset.score);y.forEach(a=>a.classList.toggle("filled",Number(a.dataset.score)<=t))}),e.addEventListener("click",()=>void H(Number(e.dataset.score)))}),v.addEventListener("mouseleave",()=>y.forEach(e=>e.classList.remove("filled"))),q.addEventListener("click",()=>void H(null)),p.addEventListener("click",()=>L(!w)),j.addEventListener("click",()=>{F()||L(!1)}),p.addEventListener("animationend",e=>{e.animationName==="sonarPing"&&p.classList.remove("invite")});function c(e,t){let a=document.createElement("div");a.className=`row ${e}`;let o=document.createElement("div");o.className=`bubble ${e}`,o.textContent=t;let n=document.createElement("div");n.className="timestamp",n.textContent=new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),a.append(o,n),b.appendChild(a),b.scrollTop=b.scrollHeight}function W(e){g.hidden=!1,g.innerHTML="";let t=document.createElement("div");t.className="confirmation-text",t.textContent=e.confirmationPrompt;let a=document.createElement("div");a.className="confirmation-actions";let o=document.createElement("button");o.textContent="Confirm",o.className="confirm",o.onclick=()=>{g.hidden=!0,B("",{confirmToolCallId:e.toolCallId})};let n=document.createElement("button");n.textContent="Cancel",n.className="cancel",n.onclick=()=>{g.hidden=!0,c("agent","No problem, I won't go ahead with that.")},a.append(o,n),g.append(t,a)}async function B(e,t){if(!i)return;e&&c("customer",e);let a=document.createElement("div");a.className="row agent",a.innerHTML='<div class="bubble agent typing"><span></span><span></span><span></span></div>',b.appendChild(a),b.scrollTop=b.scrollHeight;try{let o=await fetch(`${m}/v1/chat/${l}/messages`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${i.widgetToken}`},body:JSON.stringify({conversationId:s,message:e||"(customer confirmed the pending action)",customerIdentifier:{type:"widget_session_cookie",value:I},...t})});if(a.remove(),!o.ok){c("agent","Sorry, I'm having trouble responding right now. Please try again shortly.");return}let n=await o.json();s=n.conversationId,C(),n.reply&&c("agent",n.reply),n.pendingConfirmation&&W(n.pendingConfirmation),n.humanTakeoverActive?A():G()}catch{a.remove(),c("agent","Sorry, I'm having trouble responding right now. Please try again shortly.")}}let k,T;function A(){k||(k=setInterval(J,4e3))}function G(){clearInterval(k),k=void 0}async function J(){if(!(!s||!i))try{let e=await fetch(`${m}/v1/chat/${l}/conversations/${s}/messages`,{headers:{authorization:`Bearer ${i.widgetToken}`}});if(!e.ok)return;let t=await e.json();if(T===void 0){T=t[t.length-1]?.id;return}let a=t.findIndex(n=>n.id===T),o=a>=0?t.slice(a+1):[];for(let n of o)(n.role==="staff"||n.role==="agent")&&c("agent",n.content);t.length>0&&(T=t[t.length-1].id)}catch{}}Y.addEventListener("submit",e=>{e.preventDefault();let t=u.value.trim();t&&(u.value="",$.disabled=!0,B(t))});function K(){d.style.display="",setTimeout(()=>{w||p.classList.add("invite")},900)}async function _(){for(let e of[0,2e3,6e3]){e&&await new Promise(t=>setTimeout(t,e));try{await D(),K();return}catch(t){let a=t.status;if(a!==void 0&&a>=400&&a<500)break}}console.warn("[chat-widget] this assistant is unavailable right now, so the chat button is hidden.")}_();function Q(e){let t=e==="left"?"left":"right",a=e==="left"?"right":"left";return`
      ...
      :host {
        ...
        ${Object.entries(P.LIGHT).map(([o,n])=>`${o}: ${n};`).join(" ")}
      }
