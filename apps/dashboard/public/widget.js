"use strict";(()=>{var H={DARK:{"--panel-bg":"#0c0d13","--text-primary":"#ffffff","--text-secondary":"rgba(255,255,255,0.45)","--text-muted":"rgba(255,255,255,0.28)","--header-border":"rgba(255,255,255,0.07)","--bubble-agent-bg":"rgba(255,255,255,0.06)","--bubble-agent-border":"rgba(255,255,255,0.06)","--bubble-agent-text":"#f2f3f7","--input-bg":"rgba(255,255,255,0.06)","--input-border":"rgba(255,255,255,0.1)","--input-focus-bg":"rgba(255,255,255,0.08)","--input-text":"#ffffff","--input-placeholder":"rgba(255,255,255,0.32)","--composer-border":"rgba(255,255,255,0.07)","--composer-bg":"rgba(255,255,255,0.02)","--close-icon":"rgba(255,255,255,0.4)","--close-hover-bg":"rgba(255,255,255,0.08)","--footer-text":"rgba(255,255,255,0.25)","--footer-brand-text":"rgba(255,255,255,0.4)","--scrollbar-thumb":"rgba(255,255,255,0.12)","--panel-shadow":"0 24px 70px rgba(0,0,0,0.5), 0 4px 16px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.07) inset","--header-glow":"rgba(114,136,255,0.25)"},LIGHT:{"--panel-bg":"#ffffff","--text-primary":"#0f1330","--text-secondary":"rgba(15,19,48,0.55)","--text-muted":"rgba(15,19,48,0.4)","--header-border":"rgba(15,19,48,0.08)","--bubble-agent-bg":"rgba(15,19,48,0.045)","--bubble-agent-border":"rgba(15,19,48,0.07)","--bubble-agent-text":"#0f1330","--input-bg":"rgba(15,19,48,0.035)","--input-border":"rgba(15,19,48,0.12)","--input-focus-bg":"rgba(15,19,48,0.05)","--input-text":"#0f1330","--input-placeholder":"rgba(15,19,48,0.38)","--composer-border":"rgba(15,19,48,0.08)","--composer-bg":"rgba(15,19,48,0.015)","--close-icon":"rgba(15,19,48,0.45)","--close-hover-bg":"rgba(15,19,48,0.06)","--footer-text":"rgba(15,19,48,0.32)","--footer-brand-text":"rgba(15,19,48,0.5)","--scrollbar-thumb":"rgba(15,19,48,0.14)","--panel-shadow":"0 24px 70px rgba(20,27,77,0.16), 0 4px 16px rgba(20,27,77,0.1), 0 0 0 1px rgba(15,19,48,0.06) inset","--header-glow":"rgba(114,136,255,0.14)"}};(function(){let u=document.currentScript,p=u?.dataset.agentId;if(!p){console.error("[chat-widget] missing data-agent-id on the embed <script> tag.");return}let w=u?.dataset.apiBase??new URL(u?.src??"",location.href).origin,B=u?.dataset.position==="left"?"left":"right",L=`chat-agent:${p}`,f=JSON.parse(localStorage.getItem(L)??"{}"),T=f.sessionCookie??crypto.randomUUID(),s=f.conversationId,k=f.csatSubmittedFor;function S(){localStorage.setItem(L,JSON.stringify({conversationId:s,sessionCookie:T,csatSubmittedFor:k}))}let m=document.createElement("div");m.id="chat-agent-widget-root",document.body.appendChild(m);let n=m.attachShadow({mode:"open"});n.innerHTML=`
  ...
  `;let c=n.querySelector(".launcher"),h=n.querySelector(".panel"),N=n.querySelector(".close"),l=n.querySelector(".messages"),b=n.querySelector(".confirmation"),x=n.querySelector(".csat"),v=n.querySelectorAll(".csat-star"),P=n.querySelector(".csat-skip"),q=n.querySelector(".composer"),g=n.querySelector("input"),C=n.querySelector(".composer button"),j=n.querySelector(".name"),I=n.querySelector(".avatar"),Y=n.querySelector(".avatar-fallback");g.addEventListener("input",()=>{C.disabled=g.value.trim().length===0});let i,y=!1;function U(e){let t=H[e??"DARK"];for(let[a,r]of Object.entries(t))m.style.setProperty(a,r)}async function D(){let e=await fetch(`${w}/v1/widget-config/${p}`);if(!e.ok)throw new Error(`widget-config fetch failed: ${e.status}`);i=await e.json(),U(i.theme),j.textContent=i.name;let t=i.logoUrl||i.avatarUrl;t&&(I.src=t,I.hidden=!1,Y.hidden=!0),f.conversationId||d("agent",i.greeting)}let M;function E(e){y=e,clearTimeout(M),c.classList.toggle("open",e),e&&c.classList.remove("invite"),e?(h.hidden=!1,requestAnimationFrame(()=>h.classList.add("open")),g.focus()):(h.classList.remove("open"),M=setTimeout(()=>{y||(h.hidden=!0)},200))}function R(){return!s||k===s||!x.hidden?!1:(x.hidden=!1,v.forEach(e=>e.classList.remove("filled")),!0)}async function $(e){if(x.hidden=!0,e!==null&&s&&i){k=s,S();try{await fetch(`${w}/v1/chat/${p}/conversations/${s}/csat`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${i.widgetToken}`},body:JSON.stringify({score:e})})}catch{}}E(!1)}v.forEach(e=>{e.addEventListener("mouseenter",()=>{let t=Number(e.dataset.score);v.forEach(a=>a.classList.toggle("filled",Number(a.dataset.score)<=t))}),e.addEventListener("click",()=>void $(Number(e.dataset.score)))}),x.addEventListener("mouseleave",()=>v.forEach(e=>e.classList.remove("filled"))),P.addEventListener("click",()=>void $(null)),c.addEventListener("click",()=>E(!y)),N.addEventListener("click",()=>{R()||E(!1)}),setTimeout(()=>{y||c.classList.add("invite")},900),c.addEventListener("animationend",e=>{e.animationName==="sonarPing"&&c.classList.remove("invite")});function d(e,t){let a=document.createElement("div");a.className=`row ${e}`;let r=document.createElement("div");r.className=`bubble ${e}`,r.textContent=t;let o=document.createElement("div");o.className="timestamp",o.textContent=new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),a.append(r,o),l.appendChild(a),l.scrollTop=l.scrollHeight}function O(e){b.hidden=!1,b.innerHTML="";let t=document.createElement("div");t.className="confirmation-text",t.textContent=e.confirmationPrompt;let a=document.createElement("div");a.className="confirmation-actions";let r=document.createElement("button");r.textContent="Confirm",r.className="confirm",r.onclick=()=>{b.hidden=!0,z("",{confirmToolCallId:e.toolCallId})};let o=document.createElement("button");o.textContent="Cancel",o.className="cancel",o.onclick=()=>{b.hidden=!0,d("agent","No problem, I won't go ahead with that.")},a.append(r,o),b.append(t,a)}async function z(e,t){if(!i)return;e&&d("customer",e);let a=document.createElement("div");a.className="row agent",a.innerHTML='<div class="bubble agent typing"><span></span><span></span><span></span></div>',l.appendChild(a),l.scrollTop=l.scrollHeight;try{let r=await fetch(`${w}/v1/chat/${p}/messages`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${i.widgetToken}`},body:JSON.stringify({conversationId:s,message:e||"(customer confirmed the pending action)",customerIdentifier:{type:"widget_session_cookie",value:T},...t})});if(a.remove(),!r.ok){d("agent","Sorry, I'm having trouble responding right now. Please try again shortly.");return}let o=await r.json();s=o.conversationId,S(),o.reply&&d("agent",o.reply),o.pendingConfirmation&&O(o.pendingConfirmation)}catch{a.remove(),d("agent","Sorry, I'm having trouble responding right now. Please try again shortly.")}}q.addEventListener("submit",e=>{e.preventDefault();let t=g.value.trim();t&&(g.value="",C.disabled=!0,z(t))}),D();function A(e){let t=e==="left"?"left":"right",a=e==="left"?"right":"left";return`
      ...
      :host {
        ...
        ${Object.entries(H.DARK).map(([r,o])=>`${r}: ${o};`).join(" ")}
      }
