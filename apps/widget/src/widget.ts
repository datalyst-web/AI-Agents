...
(function bootstrap() {
  ...
  const position: "left" | "right" = currentScript?.dataset.position === "left" ? "left" : "right";
  ...
  function styles(position: "left" | "right"): string {
    ...
    // predates this option); data-position="left" mirrors both the
    // launcher and the open panel, including which corner the panel's
    // "grows out of" animation originates from, so it doesn't look like
    // it's sliding in from the wrong side.
    ...
    return `
      ...
      .panel {
        position: fixed; bottom: 96px; ${side}: 24px; ${otherSide}: auto; width: 372px; max-width: calc(100vw - 32px); height: 580px;
      ...
      @media (max-width: 480px) {
        ...
        .launcher { bottom: 18px; ${side}: 18px; ${otherSide}: auto; }
      }
    `;
  }
})();
